  ...
  const getData = async () => {
    try {
      const res = await axios.get(
        "https://api.dexscreener.com/latest/dex/tokens/0x28561B8A2360F463011c16b6Cc0B0cbEF8dbBcad"
      );
      if (res?.data?.pairs?.[0]) {
      ...
      <div className={clsx("relative", "px-5 lg:px-10", "py-4")}>
        ...
        >
          <section>
            ...
            <div className="">
              ${pair ? Number(pair?.marketCap).toLocaleString("en") : 0}
            </div>
          </section>
          <section>
            <div className="">LIQUIDITY</div>
            <div className="">
              ${pair ? Number(pair?.liquidity?.usd).toLocaleString("en") : 0}
            </div>
          </section>
          <section>
            <div className="">VOLUME (24h)</div>
            <div className="">
              ${pair ? Number(pair?.volume?.h24).toLocaleString("en") : 0}
            </div>
          </section>
        </div>
        <div
